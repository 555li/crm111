...
<script type="text/javascript">
	...
	$(function(){
		...
		// 	//如果列表中的所有chechbox都选中，则"全选"按钮也选中
		// 	if($("#tBody input[type='checkbox']").size()==$("#tBody input[type='checkbox']:checked").size()){
		// 		$("#checkAll").prop("checked",true);
		// 	}
		// 	else{//如果至少有一个没选中，则全选按钮也取消
		...
		$("#importActivityBtn").click(function(){
			...
			$.ajax({
				...
				data:formData,
				processData:false,//设置ajax向后台提交参数之后，是否把参数统一转成字符串，默认是true
				contentType:false,//设置ajax向后台提交参数之后，是否把参数统一按urlencoded编码，默认是true
				type:'post',
				dataType:'json',
				success:function(data){
					if(data.code==1){
	...
	function queryActivityByConditionForPage(pageNo,pageSize){
		//当市场活动页面加载完成后，查询所有数据的第一页以及所有数据的总条数
		//收集参数
		...
		//发送请求
		$.ajax({
			url:"workbench/activity/queryActivityByCondtionForPage.do",
			data:{
				name:name,
				owner:owner,
				startDate:startDate,
		...
		<div style="width: 100%; position: absolute;top: 5px; left: 10px;">
			...
			<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 5px;">
				<div class="btn-group" style="position: relative; top: 18%;">
				  ...
				  <button type="button" class="btn btn-default" data-toggle="modal" data-target="#editActivityModal" id="updateActivityBtn"><span class="glyphicon glyphicon-pencil"></span> 修改</button>
